...
export const scheduleViewing = async (req, res) => {
  try {
    ...
    // Notify listed property owner when a new request is created
    if (property.postedBy && (!req.user || property.postedBy.toString() !== req.user._id.toString())) {
      try {
        const seller = await User.findById(property.postedBy);
        if (seller?.email && seller.email !== recipientEmail) {
          ...
          transporter.sendMail(ownerMailOptions).catch(emailErr => {
            console.error('Background owner notification email failed:', emailErr.message);
          });
        }
      } catch (ownerEmailErr) {
        console.error('Owner notification email dispatch error:', ownerEmailErr.message);
      }
    }
...
export const getAppointmentsBySeller = async (req, res) => {
  try {
    const sellerPropertyIds = await Property.find({ postedBy: req.user._id }).select('_id');
    const propertyIds = sellerPropertyIds.map((property) => property._id);
    ...
    res.status(500).json({
      success: false,
      message: 'Error fetching seller appointments'
    });
  }
};

export const updateSellerAppointmentStatus = async (req, res) => {
  try {
    const { appointmentId, status } = req.body;
    ...
    if (!appointment.propertyId || appointment.propertyId.postedBy.toString() !== req.user._id.toString()) {
      return res.status(403).json({
        success: false,
    ...
    if (recipientEmail) {
      try {
        ...
        transporter.sendMail(mailOptions).catch(emailErr => {
          console.error('Background seller status email failed:', emailErr.message);
        });
      } catch (emailErr) {
        console.error('Seller status email dispatch error:', emailErr.message);
      }
    }
    ...
    console.error('Error updating seller appointment status:', error);
    res.status(500).json({
      success: false,
      message: 'Error updating appointment'
    });
