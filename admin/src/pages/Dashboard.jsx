...
const QuickActionCard = ({ title, count, icon: Icon, accent, to, index }) => (
  ...
  >
    <Link
      to={to}
      className="block bg-white rounded-xl p-4 border border-[#E6D5C3] shadow-card hover:shadow-card-hover hover:border-[#2563EB] transition-all duration-200 group"
    >
      ...
      <div className="mt-3 flex items-center text-xs font-medium text-[#2563EB] group-hover:text-[#C05E44] transition-colors">
        View details
...
const ActivityItem = ({ item }) => {
  ...
  const getActionIcon = () => {
    if (isAdminLog) {
      ...
      if (item.action.includes('property')) return <Building2 className="w-4 h-4 text-[#2563EB]" />;
    }
  ...
  const getActionColor = () => {
    if (isAdminLog) {
      ...
      if (item.action.includes('property')) return "bg-[#2563EB]/10";
    }

    return isProperty ? "bg-[#2563EB]/10" : "bg-blue-50";
  };
  ...
  const statCards = [
    {
      ...
      icon: Home,
      accent: { bg: "bg-[#2563EB]/10", icon: "text-[#2563EB]" },
      ...
      <div className="min-h-screen flex items-center justify-center bg-[#FAF8F4] pt-8">
        <div className="text-center max-w-md">
          ...
          <button onClick={() => fetchStats()}
            className="px-6 py-3 bg-[#2563EB] text-white rounded-xl font-semibold text-sm hover:bg-[#C05E44] transition-colors">
            Try Again
          </button>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto pt-8">
        ...
        <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }}
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            className="flex items-center gap-2 px-4 py-2.5 bg-white border border-[#E6D5C3] text-[#1C1B1A] rounded-xl text-sm font-medium hover:border-[#2563EB] hover:text-[#2563EB] transition-all duration-200 shadow-card disabled:opacity-60 disabled:cursor-not-allowed"
          >
        ...
        >
          <div className="flex items-center justify-between mb-5">
            ...
            <Link
              to="/activity-logs"
              className="text-xs text-[#2563EB] hover:text-[#C05E44] font-medium transition-colors"
            >
              View all
            </Link>
          </div>
