  ...
  >
    ...
    <div className="p-4">
      ...
      <div className="flex items-center gap-3 text-xs text-[#5A5856] mb-4">
        <span className="flex items-center gap-1"><BedDouble className="w-3.5 h-3.5 text-[#2563EB]" />{property.beds} Beds</span>
        <span className="flex items-center gap-1"><Bath className="w-3.5 h-3.5 text-[#2563EB]" />{property.baths} Baths</span>
        <span className="flex items-center gap-1"><Maximize className="w-3.5 h-3.5 text-[#2563EB]" />{property.sqft} sqft</span>
      </div>

      {/* Price + Actions */}
      <div className="flex items-center justify-between pt-3 border-t border-[#F5F1E8]">
        <div>
          <span className="text-lg font-bold text-[#2563EB]">{formatPrice(property.price)}</span>
          {property.availability === "rent" && <span className="text-xs text-[#9CA3AF] ml-1">/mo</span>}
        </div>
        <div className="flex items-center gap-1.5">
          <Link to={`/update/${property._id}`}>
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
              className="p-2 bg-[#FAF8F4] border border-[#E6D5C3] text-[#5A5856] rounded-lg hover:border-[#2563EB] hover:text-[#2563EB] transition-all duration-200">
...
const PropertyListRow = ({ property, onRemove }) => (
  <motion.div
    ...
    exit={{ opacity: 0, x: -10 }}
    className="flex items-center gap-4 p-4 bg-white rounded-xl border border-[#E6D5C3] hover:border-[#2563EB]/30 hover:shadow-card transition-all duration-200"
  >
    {/* Thumbnail */}
    ...
    <div className="hidden md:flex items-center gap-4 text-xs text-[#5A5856]">
      <span className="flex items-center gap-1"><BedDouble className="w-3.5 h-3.5 text-[#2563EB]" />{property.beds}</span>
      <span className="flex items-center gap-1"><Bath className="w-3.5 h-3.5 text-[#2563EB]" />{property.baths}</span>
      <span className="flex items-center gap-1"><Maximize className="w-3.5 h-3.5 text-[#2563EB]" />{property.sqft}</span>
    </div>

    {/* Price */}
    <div className="text-right flex-shrink-0">
      <div className="font-bold text-[#2563EB] text-sm">{formatPrice(property.price)}</div>
      <div className={cn(
    ...
    <div className="flex items-center gap-1.5 flex-shrink-0">
      <Link to={`/update/${property._id}`}>
        <button className="p-2 text-[#5A5856] hover:text-[#2563EB] hover:bg-[#2563EB]/10 rounded-lg transition-all duration-200">
          <Edit3 className="w-4 h-4" />
        </button>
      </Link>
    ...
    return (
      <div className="min-h-screen pt-8 flex items-center justify-center bg-[#FAF8F4]">
        <div className="text-center">
          <div className="w-12 h-12 border-3 border-[#2563EB] border-t-transparent rounded-full animate-spin mx-auto mb-4" />
        ...
        <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div>
            ...
            <p className="text-[#5A5856] text-sm">
              <span className="font-semibold text-[#2563EB]">{filteredProperties.length}</span> listings found
            </p>
          </div>
          <div className="flex items-center gap-2">
            <motion.button onClick={handleRefresh} disabled={refreshing}
              whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
              className="flex items-center gap-2 px-4 py-2.5 bg-white border border-[#E6D5C3] text-[#1C1B1A] rounded-xl text-sm font-medium hover:border-[#2563EB] hover:text-[#2563EB] transition-all shadow-card disabled:opacity-60">
              <RefreshCw className={cn("w-4 h-4", refreshing && "animate-spin")} />
              <span className="hidden sm:inline">Refresh</span>
            </motion.button>
            <Link to="/add">
              <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
                className="flex items-center gap-2 px-5 py-2.5 bg-[#2563EB] hover:bg-[#C05E44] text-white rounded-xl text-sm font-semibold transition-all shadow-lg hover:shadow-terracotta">
          ...
          <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
            ...
            <div className="relative flex-1 w-full">
              ...
              <input type="text" placeholder="Search properties..."
                ...
                className="w-full pl-9 pr-4 py-2.5 bg-[#FAF8F4] border border-[#E6D5C3] rounded-xl text-sm text-[#1C1B1A] placeholder-[#9CA3AF] outline-none focus:border-[#2563EB] focus:ring-2 focus:ring-[#2563EB]/15 transition-all" />
            </div>
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              <select value={sortBy} onChange={(e) => setSortBy(e.target.value)}
                className="px-3 py-2.5 bg-[#FAF8F4] border border-[#E6D5C3] rounded-xl text-xs text-[#5A5856] outline-none focus:border-[#2563EB] transition-all cursor-pointer">
          ...
          {filteredProperties.length === 0 ? (
            <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              {!searchTerm && filterType === "all" && (
                <Link to="/add">
                  <button className="px-6 py-3 bg-[#2563EB] text-white rounded-xl font-semibold text-sm hover:bg-[#C05E44] transition-colors">
                    Add First Property
                  </button>
                </Link>
              )}
